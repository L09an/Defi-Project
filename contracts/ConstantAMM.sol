...
contract ConstantAMM is ERC20("Constant LP Token", "CLPT") {
    ...
    function calculateIn(uint256 tokenOut, uint256 tokenInBalance, uint256 tokenOutBalance) internal pure returns (uint256) {
        uint256 fee = 3; // 0.3%
        ...
        uint256 invariant = scaledTokenInBalance * scaledTokenOutBalance;
        uint256 newScaledTokenInBalance = invariant/ newScaledTokenOutBalance;
    ...
    function calculateOut(uint256 tokenIn, uint256 tokenInBalance, uint256 tokenOutBalance) internal pure returns (uint256) {
        ...
        //fee多除了1000
        uint256 newTokenOutBalance = (invariant) / (newTokenInBalance);
        uint256 tokenOut = scaledTokenOutBalance.sub(newTokenOutBalance).mul(1e8);
        return tokenOut;
    }

    function tokenWithoutFee(uint256 tokenIn, uint256 tokenInBalance, uint256 tokenOutBalance) internal pure returns (uint256){
        uint256 scaledTokenInBalance0 = tokenInBalance.div(1e8);
        uint256 scaledTokenOutBalance0 = tokenOutBalance.div(1e8);
        uint256 invariant0 = (scaledTokenInBalance0 * scaledTokenOutBalance0);
        uint256 newTokenInBalance0 = scaledTokenInBalance0.add(tokenIn.div(1e8));
        uint256 newTokenOutBalance0 = (invariant0 ) / (newTokenInBalance0);
